 ...
 * 数据稀疏性问题：
 * 用神经网络进行稀疏路段行程时间预测：
 * 每天的数据按照半小时内的数据求期望与方差，而不是将所有的数据都合并为一起后再求期望与方差
 * 这样神经网络训练的数据明显增多，增加数据的异构性
 * 这样神经网络训练的效果很不好，已经试验了
 * 输入：
 * 输出：
 * @author whu
 *
 */
public class deleteAdaptMainFunctionANNInputLengthDegreeRatioExpectationDeviation {

	public static void main(String[] args) {
//		new AdaptMainFunctionANNInputLengthDegreeRatioExpectationDeviation().allLinkTravelCharacteristicCalculation();//计算所有路段速度期望、速度方差
		
		new deleteAdaptMainFunctionANNInputLengthDegreeRatioExpectationDeviation().allLinkANNInputAndNormalizeInput();
...
//		Integer[]IDArray = {12,21,25,33,44,52,58,66,72,77,82,87,88};
		...
		for (int i = 0; i < IDArray.length; i++) {
			...
			new deleteAdaptMainFunctionANNInputLengthDegreeRatioExpectationDeviation().obtainANNInputAndNormalizeInput(linkID);
		}
		System.out.print("done!");
	}
...
//								 "21:00:00","21:30:00","22:00:00","22:30:00","23:00:00","23:30:00"};
		...
		while (!tempStartDateTimeStr.equals(endDateTimeStr)) {				
			...
			timeArraylist.add(tempStartDateTimeStr);
		...
		for (int i = 0; i < timeArraylist.size(); i++) {
			timeStr = timeArraylist.get(i);
			ANNInput(linkID, timeStr, targetAdjacentLinkTravelTimeFilePathStr, inputArrayList, infosArrayList, polylineCollArrayList);
			System.out.print(i + ":" + timeArraylist.size());
		}
...
//				linkID = 1;
	...
	public void speedDeviationCalculation(int linkID, ArrayList<LinkTravelCharacteristic> speedExpectationStandardDeviationArrayList,ArrayList<String> infosArrayList) {
		try {
			Map<String, Double> expectedSpeedMap = new HashMap<String, Double>();//时间为索引，存储数据
//			expectedSpeedcalculation(linkID,infosArrayList,expectedSpeedMap);			
			...
			while (!tempStartDateTimeStr.equals(endDateTimeStr)) {				
				...
				if (expectedSpeed != 0) {
					...
					linkTravelCharacteristic.setTimeStr(tempStartDateTimeStr);
					...
					System.out.print(tempStartDateTimeStr + "," + expectedSpeed + ":" + speedStandardDeviation + '\n');
