 ...
 *
 */
public class deleteMainFunctionANNInputLengthDegreeRatioPearsonCoefficient {

	public static void main(String[] args) {
//		new MainFunctionANN().allLinkTravelCharacteristicCalculation();
//		new MainFunctionANN().ANNInput(12, "10:30:00", "");
		new deleteMainFunctionANNInputLengthDegreeRatioPearsonCoefficient().allLinkANNInputAndNormalizeInput();
...
//		Integer[]IDArray = {12,21,25,33,44,52,58,66,72,77,82,87,88};
		...
		for (int i = 0; i < IDArray.length; i++) {
			...
			new deleteMainFunctionANNInputLengthDegreeRatioPearsonCoefficient().obtainANNInputAndNormalizeInput(linkID);
		}
		System.out.print("done!");
	}
	...
	public void obtainANNInputAndNormalizeInput(int linkID) {
		...
		String headheadDescriptionStr = "similarity " + "," + "lengthRate" + "," + "pearsonCorr" + "," + "travelTimeRate" + "\r\n";
...
//				double travelTime = obtainTravelTimeAccordLinkID(targetLinkID, timeStr, infosArrayList);//行程时间
				
				if (Math.abs(similarity) > 0.000001 && Math.abs(lengthRate) > 0.000001 && Math.abs(pearsonCorr) > 0.000001 && Math.abs(travelTimeRate) > 0.000001) {
					double[]inputInfos = new double[4];
					inputInfos[0] = similarity;
					inputInfos[1] = lengthRate;
					inputInfos[2] = pearsonCorr;
					inputInfos[3] = travelTimeRate;
