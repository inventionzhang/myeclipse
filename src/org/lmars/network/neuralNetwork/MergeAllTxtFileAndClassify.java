...
//							System.out.print(str + "\n");
				 ...
				 * 2015/9/18日修改
				 */
				ArrayList<String> allMergerInfosArrayList = new ArrayList<String>();
				readFromTxtFile(mergeFileOutputPath, allMergerInfosArrayList);//对于大数据量来说，内存溢出，所以要设置堆栈
				for (int i = 0; i < IDArrayList.size(); i++) {	
					String IDStr = IDArrayList.get(i);
					ArrayList<String> infosArrayList = new ArrayList<String>();
					readFromTxtFileAccordIDAndDirection(mergeFileOutputPath, IDStr, allMergerInfosArrayList,infosArrayList);
	 ...
	 *  从文件中读取的每一条记录与ID进行比较
	 * @param IDStr
	 * @param allMergerInfosArrayList	文件中的所有合并信息
	 * @param infosArrayList 与ID相同信息
	 * @param filePathStr
	 */
	public void readFromTxtFileAccordIDAndDirection(String filePathStr, String IDStr, ArrayList<String> allMergerInfosArrayList,
		...
		try {
			...
			for (int j = 0; j < allMergerInfosArrayList.size(); j++) {
				String str = allMergerInfosArrayList.get(j);
				if (str != null) {
					String[]tempArrayStr = str.split(",");
