...
import org.lmars.network.sparseData.LinkTravelCharacteristic;
...
public class FileOperateFunction {
	...
	public static void readFromTxtFile(String filePathStr, List<String> infosArrayList){		
		try {
			...
			if (file.exists()) {
				...
				reader.close();
				System.out.print("txt文件读取结束:" + filePathStr + "\n");
			}
			else {
...
//			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr),true);//写入文件末尾处
	 ...
	 * @param infosArrayList
	 */
	public static void writeStringArrayToTxtFile(String filePathStr, String headDescriptionStr, ArrayList<String[]> infosArrayList) {
		try {
			System.out.print("开始写入txt文件");	
			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr));//写入文件开始处,信息覆盖
//			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr),true);//写入文件末尾处
			...
			bufferStream.write(write.toString().getBytes("UTF-8"));
			for (int i = 0; i < infosArrayList.size(); i++) {
				String [] temp = infosArrayList.get(i);
				String tempStr = "";
				for (int j = 0; j < temp.length; j++) {
					if (j == 0 ) {
						tempStr = String.valueOf(temp[0]);
...
//			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr),true);//写入文件末尾处
	 ...
	 * @param infosList
	 */
	public static void writeStringListToTxtFile(String filePathStr, String headDescriptionStr, List<String> infosList) {
		try {
			System.out.print("开始写入txt文件");	
			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr));//写入文件开始处,信息覆盖
//			FileOutputStream outputStream = new FileOutputStream(new String(filePathStr),true);//写入文件末尾处
